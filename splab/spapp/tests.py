...
class DashboardPlannerTests(TestCase):
    ...
    def test_dashboard_includes_planner_items_for_upcoming_events(self):
        ...
        self.assertIn('Focus next on', response.context['focus_message'])

    def test_dashboard_marks_recently_overdue_assignments(self):
        Assignment.objects.create(
            course=self.course,
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['planner_items']), 1)
